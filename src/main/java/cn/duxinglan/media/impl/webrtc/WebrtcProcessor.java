 ...
 /**
  * WebrtcProcessor 类是一个用于处理 WebRTC 相关功能的核心组件。
  * 它负责管理 WebRTC 会话的信令交互、媒体流配置以及 RTP 负载类型的初始化和协商。
  * 此类在支持 WebRTC 数据流传输以及会话描述生成方面扮演重要角色。
  */
 @Data
 @Slf4j
 public class WebrtcProcessor {
     ...
     private boolean isUpdateMediaLine = false;

     public WebrtcProcessor(DTLSKeyMaterial keyMaterial, IWebrtcProcessorEvent webrtcProcessorEvent) throws Exception {
     ...
     private void sendOffer() {
         synchronized (lock) {
             if (isSend && isUpdateMediaLine) {
                 try {
                     RTCSessionDescriptionInit offer = createOffer();
                     isSend = false;
                     isUpdateMediaLine = false;
                     webrtcProcessorEvent.onSendOffer(offer);
                 } catch (Exception e) {
                     log.error(e.getMessage(), e);
                 }
             }
     ...
     public void setRemoteDescription(SessionDescription remoteSessionDescription) {
         ...
         webrtcProcessorEvent.onIceInfo(localIceInfo.getLocalIceInfo(), localIceInfo.getRemoteIceInfo());
         webrtcProcessorEvent.onRemoteMediaLinesParsed(mediaLineInfoMap.values());

     }


     public MediaLineInfo onAddSendMediaLineInfo(MediaInfoType type, MediaLineInfo.Info info) {
         MediaLineInfo mediaLineInfo = null;
         synchronized (lock) {
             mediaLineInfo = new MediaLineInfo(type, getMid(), true, false);
             mediaLineInfo.setSendInfo(info);
             mediaLineInfoMap.put(mediaLineInfo.getMid(), mediaLineInfo);
             isUpdateMediaLine = true;
             sendOffer();
...
//                mediaSb.append(String.format("a=rtpmap:%d %s/%d", rtpPayload.getPayloadType(), rtpPayload.getEncodingName(), rtpPayload.getClockRate())).append("\r\n");
     ...
     public interface IWebrtcProcessorEvent {
         ...
         void onRemoteMediaLinesParsed(Collection<MediaLineInfo> mediaLines);
         ...
         /**
          * 此方法用于处理本地和远端的 ICE 信息。
          *
          * @param localIceInfo  本地的 ICE 信息，包括 ufrag、pwd 和其他可选数据。
          * @param remoteIceInfo 远端的 ICE 信息，包括 ufrag、pwd 和其他可选数据。
          */
         void onIceInfo(IceInfo localIceInfo, IceInfo remoteIceInfo);

         void onSendOffer(RTCSessionDescriptionInit offer);
     }
