...
import cn.duxinglan.media.signaling.sdp.ssrc.SsrcGroupType;
 ...
 **/
public class SsrcGroupLineParser extends MediaLineParser {

    public static final String KEY = "ssrc-group";
    ...
    protected boolean parse(MediaDescription mediaDescription, String key, String value) {
        ...
        SsrcGroupType ssrcGroupType = SsrcGroupType.fromValue(s[0]);
        SsrcGroup ssrcGroup = new SsrcGroup();
        ssrcGroup.setSsrcGroupType(ssrcGroupType);
        for (int i = 1; i < s.length; i++) {
            String string = s[i];
            ssrcGroup.addSsrc(Long.parseLong(string));
        }
        mediaDescription.addSsrcGroup(ssrcGroup);
        return true;
    }
}
