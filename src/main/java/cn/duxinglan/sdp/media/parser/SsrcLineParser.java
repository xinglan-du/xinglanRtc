 ...
 **/
public class SsrcLineParser extends MediaLineParser {
    ...
    protected boolean parse(MediaDescription mediaDescription, String key, String value) {
        ...
        Long ssrcId = Long.parseLong(s[0]);
        SSRC ssrc = mediaDescription.getSsrcMap().get(ssrcId);
        if (ssrc == null) {
            ssrc = new SSRC();
            ssrc.setSsrc(ssrcId);
            mediaDescription.getSsrcMap().put(ssrcId, ssrc);
        }

        String[] s1 = s[1].split(":");
        if ("cname".equals(s1[0])) {
            ssrc.setCname(s1[1]);
        } else if ("msid".equals(s1[0])) {
            ssrc.setStreamId(s1[1]);
            ssrc.setTrackId(s[2]);
        }
